...
const { normalizePhone, isValidPhoneDigits, isMobileFr, phoneToJid } = require('./phones');
...
function toContact(row, source) {
  ...
  if (!isValidPhoneDigits(phone) || !isMobileFr(phone)) return null;
  return {
...
function pickSavedUnsent(count) {
  ...
  for (const contact of loadProdContacts()) {
    if (picked.length >= n) break;
    const row = state.phones[contact.telephone];
...
/** Contacts déjà destinataires du 1er message David, pas encore relancés. */
function pickWaSentForReviens(count) {
  const n = Math.max(0, parseInt(count, 10) || 0);
  const state = loadUsed();
  const byPhone = new Map(loadProdContacts().map((c) => [c.telephone, c]));
  const picked = [];
  const rows = Object.values(state.phones || {})
    .filter((row) => row && row.status === 'wa_sent' && !row.reviens_sent)
    .sort((a, b) => String(a.at || '').localeCompare(String(b.at || '')));
  for (const row of rows) {
    if (picked.length >= n) break;
    const phone = normalizePhone(row.phone || row.telephone);
    if (!isMobileFr(phone)) continue;
    ...
    picked.push({ ...withJid(contact), via: row.via || 'whatsapp' });
  }
  return picked;
}
